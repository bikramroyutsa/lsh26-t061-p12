...
export const ForecastCard: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`border-4 border-black p-5 mb-5 shadow-neo-sm flex items-center justify-between flex-wrap gap-4 ${
          forecast.is_deficit ? "bg-[#FF6B6B]/25" : "bg-[#00F0B5]/25"
        }`}
      >
        <div className="flex items-center gap-3.5">
          <div
            className={`p-2.5 border-3 border-black shadow-neo-xs ${
              forecast.is_deficit ? "bg-[#FF6B6B] text-black" : "bg-[#00F0B5] text-black"
            }`}
          >
            {forecast.is_deficit ? (
          ...
          <div>
            <span className="text-xs font-black uppercase tracking-wider block text-black/70">
              {forecast.is_deficit ? "Projected Month-End Deficit (Shortfall)" : "Projected Month-End Net Savings (Surplus)"}
            </span>
            <span
              className={`text-2xl sm:text-3xl font-black ${
                forecast.is_deficit ? "text-[#FF6B6B]" : "text-black"
              }`}
            >
              {forecast.is_deficit ? "- " : "+ "}৳
        ...
        <div className="bg-white border-2 border-black p-2.5 shadow-neo-xs text-xs font-bold">
          <span className="block text-black/60 uppercase text-[10px]">Projected Budget Used:</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {/* Daily Burn Rate */}
        <div className="border-3 border-black p-3.5 bg-[#FFFDF5] shadow-neo-xs flex flex-col gap-1">
          <div className="flex items-center gap-1.5 text-xs font-black uppercase text-black/60">
          ...
          <span className="text-xl font-black text-black">
            ...
            <span className="text-xs font-bold text-black/60">/day</span>
          </span>
